...
public class ProjectController {
    ...
    public ResponseEntity<CreateProjectRequest> CreateInitProject(@RequestBody CreateProjectRequest createProjectRequest, @PathVariable("product-id")Long productId) throws Exception {

        //진행 중인 프로젝트가 있을 시 접근 제한
        //projectService.checkProcessProject(productId); -> 하위 서비스에서 하는데 중복으로 들어가서 지움.

        projectService.createProject(createProjectRequest,productId);


        return new ResponseEntity<>(createProjectRequest, HttpStatus.CREATED);
    }
